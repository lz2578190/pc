...
class InputModel {
  ...
  void tapBlank(MouseButtons button) {
  ...
  Future<void> onMobileHp() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    int decryptJzrq = prefs.getInt("decryptJzrq") ?? 0;
    int millisecondsSinceEpoch = DateTime.now().millisecondsSinceEpoch;

    if (decryptJzrq <= 0 || decryptJzrq <= millisecondsSinceEpoch) {
      showToast("黑屏模式为高级模式，请购买授权码并激活");
    } else {
      tapBlank(MouseButtons.wheel);
    }
  }
